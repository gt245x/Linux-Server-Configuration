...
Open the terminal and chmod to 600 for the downloaded key

        chmod 600 ~/.ssh/udacity_key.rsa

In the terminal, ssh to ther server by:
...
Move the authorization key from the root user to the new user (grader) .ssh folder and change owner of the file from root to grader  

Use the usermod command to add the user (grader) to the sudo group

        usermod -aG sudo grader

Use the su command to switch to the grader account

        su grader

Verify that the grader has sudo account by prepending 'sudo' to the command that you want to run with superuser privileges like listing contents of the /root directory
...
Next, update the softwares

        sudo apt-get upgrade

Remove any packages no longer required that we can automatially remove

        sudo apt-get autoremove

Install software finger
...
SSH to the server using the graders login info
...
Verify that the firewall is still inactive

        sudo ufw status

Allow SSH at port 2200
...
After installation, check the server address and it should show the ubuntu web page saying that it's working.

Install mod-wsgi tool and python-setuptools for serving python applications
...
Configure the user name by this command

        git config --global user.name "Your Name"

Configure the user e-mail by this command
...
Install PostgreSQL by using the following command line:

        sudo apt-get install postgresql postgresql-contrib

Do not allow Remote Connections. Simplest way to avoid potential attack vector is to not allow remote connections to the database which is the current default when installing PostgreSQL.

Double check that no remote connections are allowed by looking in the host based authentication file:

        sudo nano /etc/postgresql/9.3/main/pg_hba.conf

Create a new user name catalog that has limited permission to database

        sudo adduser catalog

postgres user was created during install. Switch to postgres user:
...
Create database user with password "db-password"
...
We can now create a database 'catalog' owned by "catalog":
...
Grant permission to owner catalog

        GRANT CONNECT ON DATABASE catalog to catalog;
        GRANT ALL ON SCHEMA public to catalog;

Access privileges can be viewed by:
...
Update catalog.wsgi with the applicaton secret key.

Open the configuration file for the catalog. 

        sudo nano /etc/apache2/sites-available/catalog.conf     

Update the serverName with the ip address.
Add the ServerAlias name below the ServerName
...
If everything works fine, the catalog should show up on the webserver.

Install oauth2client 

        sudo pip install --upgrade oauth2client

Update google login authorization:
...
Update facebook login authorization:
...
Copy the existing jail.conf file to jail.local
...
Adjust bantime parameter to 1800 secs or 30 mins

        bantime = 1800

Findtime and maxrentry establish conditions for an illegitimate user to be banned.

        findtime = 600
        maxretry = 3       

Update destemail to get ban reports

        destemail = root@localhost


##15. Command to list, add, delete and modify users
